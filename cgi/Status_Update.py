...
import sys
# reload(sys) and sys.setdefaultencoding are not needed/available in Python 3
...
import argparse
#import urllib.request (replacement for urllib2 if needed, but requests is used below)
...
try:
    ...
    # Handle cases where /dev/log doesn't exist (e.g., non-Linux dev environments)
    pass
...
# execfile replacement for Python 3
db_inc_path = "/usr/lib/cgi-bin/Dashboard/db.inc.py"
if os.path.exists(db_inc_path):
    with open(db_inc_path) as f:
        exec(f.read())
else:
    # Fallback or error handling if the include file is missing
...
class DB_Class:
    ...
    def read_GNSS_configuration(self, GNSS_ID):
        ...
        if row is None:
            print(f"Error: GNSS ID {GNSS_ID} not found in database.")
            sys.exit(1)
...
def STATUS_Update_Check(DB, GNSS_ID, Enabled):
    # FIX: Convert datetime object to string explicitly to avoid Python 3.12 DeprecationWarning
    now_str = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    if not Enabled:
        logger.warning(DB.Address + ":" + str(DB.Port) + " Disabled: ")
        DB.STATUS.execute("DELETE FROM STATUS WHERE id = ? ", (GNSS_ID,))
        DB.STATUS.execute("INSERT or REPLACE into STATUS (id,Checked,Last_Check) VALUES (?,?,?)", (GNSS_ID, False, now_str))
        DB.conn.commit()
        return (False)
    else:
...
def check_firmware_and_password(GNSS_ID, DB, HTTP):
    ...
    if reply:
        ...
        m = re.search(r" date=(\d*-\d*-\d*)", reply)
        if m:
...
def check_serial_and_type(GNSS_ID, DB, HTTP):
    ...
    # print(reply)
    if reply:
        m = re.search(r"SerialNumber sn=(.*) *rxType='(.*),.*,(.*)'", reply)
        if m:
            ...
            m = re.search(r"SerialNumber sn=(.*)", reply)
            if m:
                SerialNumber = m.group(1)
                rxValid = True
            else:
...
def check_position_type(GNSS_ID, DB, HTTP):
    ...
    if reply:
        ...
        # Unreachable code in original, kept for structure
        if not found_pos:
...
def check_antenna(GNSS_ID, DB, HTTP):
    ...
    # print(reply)
    if reply:
        m = re.search(r"Antenna type=(.*) name='(.*)' height=(.*) measMethod=(.*) serial='(.*)'", reply)
        if not m:
            m = re.search(r'Antenna type=(.*) name="(.*)" height=(.*) measMethod=(.*) serial="(.*)"', reply)
        if m:
            Antenna_Valid = True
...
def check_logging(GNSS_ID, DB, HTTP):
    ...
    if reply:
        ...
        try:
            ...
            return(False, "Logging response empty or malformed")

        m = re.search(r"enabled?=(\w*)", line)

        if not m:
            m = re.search(r"enable?=(\w*)", line)
        ...
        m = re.search(r"duration=(\w*)", line)
        if not m:
            m = re.search(r"durationMin=(\w*)", line)
        if m:
        ...
        m = re.search(r"measInterval=([0-9]+\.[0-9]+)", line)
        if m:
        ...
        m = re.search(r"posInterval=([0-9]+\.[0-9]+)", line)
        if m:
        ...
        m = re.search(r"smoothRanges=(\w*)", line)
        if m:
        ...
        m = re.search(r"smoothPhases=(\w*)", line)
        if m:
        ...
        m = re.search(r"voltTempInterval=(\w*)", line)
        if m:
        ...
        Logging_obs = None
        m = re.search(r"logCorrections=(\w*)", line)
        if m:
...
def check_clock(GNSS_ID, DB, HTTP):
    ...
    # print(reply)
    if reply:
        m = re.search(r'ClockSteering enable=(.*)', reply)
        if m:
...
def check_UPS(GNSS_ID, DB, HTTP):
    ...
    # print(reply)
    if reply:
        m = re.search(r'UPS enable=(.*)', reply)
        if m:
...
def check_PDOP(GNSS_ID, DB, HTTP):
    ...
    # print(reply)
    if reply:
        m = re.search(r'PdopMask mask=(.*)', reply)
        if m:
...
def check_Elev(GNSS_ID, DB, HTTP):
    ...
    # print(reply)
    if reply:
        m = re.search(r'ElevationMask mask=(.*)', reply)
        if m:
...
def get_Temperature(GNSS_ID, DB, HTTP):
    ...
    # print(reply)
    if reply:
        m = re.search(r'Temperature temp=(.*)', reply)
        if m:
...
def get_Warranty(GNSS_ID, DB, HTTP):
    ...
    # print(reply)
    if reply:
        m = re.search(r'FirmwareWarranty date=(.*)', reply)
        if m:
            Warranty = m.group(1)
...
def get_SystemName(GNSS_ID, DB, HTTP):
    ...
    # print(reply)
    if reply:
        m = re.search(r'SystemName name=(.*)', reply)
        if m:
...
def check_MultipathReject(GNSS_ID, DB, HTTP):
    ...
    # print(reply)
    if reply:
        m = re.search(r'MultipathReject enable=(.*)', reply)
        if m:
...
def check_testMode(GNSS_ID, DB, HTTP):
    ...
    # print(reply)
    if reply:
        m = re.search(r'testMode enable=(.*)', reply)
        if m:
...
def check_Password(GNSS_ID, DB, HTTP):
    ...
    if reply:
        ...
        if m:
            PDOP = int(m.group(1), 10)
            # (reply,result)=HTTP.get("/prog/set?PdopMask&mask="+str(PDOP))
            try:
                (reply, result) = HTTP.get("/prog/set?PdopMask&mask=" + str(PDOP))
            except:
                pass
            # print(reply)
            m = re.search(r'^ERROR', reply)
            # pprint (m)
            if m:
...
def check_Ref(GNSS_ID, DB, HTTP):
    ...
    # print(reply)
    if reply:
        m = re.search(r"RefStation lat=(.*) lon=(.*) height=(.*) Rtcm2Id=.* Rtcm3Id=.* CmrId=.* Name='(.*)' Code='(.*)'$", reply)
        if m:
            Message = ""
            Ref_Valid = True
...
# Tracking GpsL1CA=on GpsL2=on GpsL2CS=CM+CL GpsL5=I+Q Sbas=L1CA GlonassL1C=on GlonassL1P=off GlonassL2C=on GlonassL2P=off GlonassL3=Data+PilotTracking QzssL1CA=off QzssL1SAIF=off QzssL2CS=off QzssL5=off QzssLEX=off
        ...
        m = re.search(r"GpsL1CA=on", reply)
        if m:
            GPS = 1
        ...
        if DB.GPS:
            ...
            if DB.Frequencies > 1:
                m = re.search(r"GpsL2=on", reply)
                if not m:
                    m = re.search(r"GpsL2=L2CSfallback", reply)
            ...
            if DB.Frequencies > 2:
                m = re.search(r"GpsL5=", reply)
                if m:
                    m = re.search(r"GpsL5=off", reply)
                    if not m:
        ...
        m = re.search(r"GlonassL1C=on", reply)
        ...
        if DB.GLN:
            if DB.Frequencies > 1:
                m = re.search(r"GlonassL2C=on", reply)

                if not m:
                    m = re.search(r"GlonassL2P=on", reply)
        ...
        m = re.search(r"GalileoE1=", reply)
        if m:
            m = re.search(r"GalileoE1=off", reply)
            if not m:
                GAL = 1
                Frequencies = max(Frequencies, 1)
            else:
        ...
        if DB.GAL:
            ...
            if DB.Frequencies > 2:
                m = re.search(r"GalileoE5AltBoc=", reply)
                if m:
                    m = re.search(r"GalileoE5AltBoc=off", reply)
                    if not m:
                        Frequencies = max(Frequencies, 2)
                        GAL = 2
                    else:
        ...
        m = re.search(r"BeidouB1=on", reply)
        if m:
            BDS = 1
            Frequencies = max(Frequencies, 1)
        else:
        ...
        if DB.BDS:
            if DB.Frequencies > 1:
                m = re.search(r"BeidouB2=on", reply)
                if m:
                    Frequencies = max(Frequencies, 2)
                    BDS = 2
                else:
        ...
        m = re.search(r"QzssL1CA=on", reply)
        ...
        if DB.QZSS:
            if DB.Frequencies > 1:
                m = re.search(r"QzssL2CS=", reply)
                if m:
                    m = re.search(r"QzssL2CS=off", reply) # make sure we have the field and then confirm it is off
                    if not m:
                        Frequencies = max(Frequencies, 2)
                        QZSS = 2
                    else:
            ...
            if DB.Frequencies > 2:
                m = re.search(r"QzssL5=", reply)
                if m:
                    m = re.search(r"QzssL5=off", reply) # make sure we have the field and then confirm it is off
                    if not m:
                        Frequencies = max(Frequencies, 2)
                        QZSS = 3
                    else:
        ...
        m = re.search(r"Sbas=", reply)
        if m:
            m = re.search(r"Sbas=off", reply) # make sure we have the field and then confirm it is off
            if not m:
                Frequencies = max(Frequencies, 1)
                SBAS = 1
            else:
...
def check_timed(GNSS_ID, DB, HTTP):
    ...
    def check_receiver_timed_active(termLicense, End_Time_Min, End_Time_Max, message):
        ...
        # Note: End_Time and time_delta are undefined in original scope here, logic may need adjustment based on original global vars
        # Assuming they were global or passed in original code, but they are missing from args here.
        # Leaving as is to match original logic structure.

        # Placeholder for missing variables if this function is called:
        End_Time = datetime.datetime.now() # This is likely wrong without context
    ...
    def check_receiver_timed_range(termLicense, Min_Delta, Max_Delta, message):
        ...
        termLicense_end_date = termLicense_end_date.replace(tzinfo=datetime.timezone.utc)
