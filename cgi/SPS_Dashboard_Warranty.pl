...
sub help() {
    print STDERR "SPS_Dashboard_Warranty.pl -host <host> [-password <password>] [-port <port>]\n";
    print STDERR "\n";
...
my $URI="http://".$host.":".$port."/prog/show?firmwareWarranty";
...
if ($content =~ /FirmwareWarranty date=(.*)/) {
       print "$1"
    }
